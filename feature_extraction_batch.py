...
def extract_eis_features(data_file, frequencies_file, battery=None, soc=None, cycle=None, choice_plot_graph=False):
    ...
    def advanced_x_intercept(xnew, ynew):
        """
        More robust method to find x-axis intercept using curve fitting and root finding.
        ...
        """
        # Use more points for robust fitting
    ...
    # Feature extraction
    # Find min curvature (not peak) using curvature analysis
    ...
    # Shoulder detection (simplified)
    shoulder = 0  # Placeholder, could be improved
...
if __name__ == "__main__":
    # File paths
    data_file = './Data/Data_IFR14500_Batt_1-11_cycle_1_2.csv'
    freq_file = 'Data/Frequencies.csv'

    # Read the dataset into a DataFrame
    df = pd.read_csv(data_file)

    # Iterate over unique combinations of Battery, SoC, and Cycle
    features_list = []
    for _, row in df.iterrows():
        ...
        features = extract_eis_features(
            data_file, freq_file, battery=battery, soc=soc, cycle=cycle, choice_plot_graph=False)
        features_list.append(features)

    # Convert the features to a DataFrame for easy storage or further processing
    features_df = pd.DataFrame(features_list)

    # Print the extracted features
    print(features_df)

    # Save features_df to a CSV file
    output_file = "./Output/extracted_features.csv"
